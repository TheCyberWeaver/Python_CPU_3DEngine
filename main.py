...
class ThreeDimensionalEngine:
    ...
    def drawGen2(self, object:Object,drawPoints=False,drawLines=True):
        ...
        if drawPoints:
            for i in range(len(normalSpace[0])):
                if mask[i]:
                    pg.draw.circle(self.screen, (255, 255, 255), [screenMatrix[0][i]/screenMatrix[3][i], screenMatrix[1][i]/screenMatrix[3][i]], 3)

        for face in object.faces:
            if np.all([mask[i] for i in face]):
                pg.draw.lines(self.screen, (255, 255, 100), True, [(screenMatrix[0][i]/screenMatrix[3][i],screenMatrix[1][i]/screenMatrix[3][i]) for i in face], 1)

    def drawGen3(self, object: Object, drawPoints=False,drawLines=True, drawFace=False):
        ...
        if drawFace:
            for face in object.faces:
                if np.all([mask[i] for i in face]):
                    for point in face:
                        if screenMatrix[2][point]:
                            pass
                    pg.draw.polygon(self.screen, (255, 255, 255), [(screenMatrix[0][i], screenMatrix[1][i]) for i in face])
    ...
    def updateScreen(self):
        ...
        for object in self.objectList:
            self.drawGen3(object,drawLines=True,drawPoints=False)
